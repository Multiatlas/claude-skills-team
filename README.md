...
| `telegram-bot-resilience` | **Que tu bot de Telegram no se caiga ni se quede "zombi".** Mata los restart-loops (`safeSend` + handlers de `polling_error`/`unhandledRejection`) y el caso traicionero del bot "online pero muerto": **watchdog de 3 barreras** (`polling_error` fatal 401/409 + healthcheck `getMe()` cada 60s + reinicio preventivo). Para todo bot `node-telegram-bot-api` con `polling` en producción. |
| `cross-surface-chat-memory` | **Que tu agente sea UNO solo en todos los canales.** El cliente te escribe por la web por la mañana y por Telegram por la tarde — y el agente recuerda toda la conversación como si fuera el mismo "asesor". Memoria única en BD (no en RAM, que se borra al reiniciar) + carga cruzada por ventana temporal. Código TS listo (singleton por canal, anti-duplicación, tolerancia a fallos). Para agentes en web + Telegram + WhatsApp. |
| `wargame-battle-plans` | **Usa un modelo barato como si fuera el caro.** Destila el criterio del mejor modelo disponible en un "plan de batalla" que pelea la misión (go-live, deploy, DNS, base de datos) movimiento a movimiento: qué debería pasar, el fallo más probable + su señal, la contramaniobra, los triggers de bifurcación y cuándo abortar. Lo ejecuta después cualquier modelo barato con ese criterio ya cocinado. Pipeline redactor → red-team (8 puntos) → reparador; ejecutable a ciegas (arranca por RECON en runtime), enlaza tus docs en vez de duplicarlos y nunca toca valores de secretos. Cuando salga un modelo mejor, re-destilas y todos tus planes suben de nivel. |
| `claude-code-sin-dar-ok` | **Deja de darle «Yes» a Claude Code a cada paso, sin quedarte sin red.** El modo sin permisos NO significa barra libre: la lista de `deny` **sigue vigente** y se fusiona entre niveles, así que puedes tener un agente trabajando solo toda la noche con `rm -rf` bloqueado. Trae las tres capas (permisos, hook de fronteras, doctrina), **la pieza de VS Code sin la cual nada de esto se aplica** (`allowDangerouslySkipPermissions`: sin ella un bypass arranca en MANUAL, y la extensión **nunca lee el settings del proyecto**), por qué el hook debe devolver `deny` y no `escalateToUser` (en bypass se ignora — nos costó un email enviado de verdad), las 4 pruebas que no se engañan (`git` y `ssh` NO valen: ya están en tu `allow`) y el aviso de que **acotar directorios no limita `bash`**. |
| `supabase-escrituras-admin-service-role` | **Tu panel dice «guardado» y la base de datos está vacía.** Con RLS activo, un endpoint admin que escribe con la clave `anon` **no lanza error**: afecta a 0 filas, devuelve `error: null`, y tu código responde `success: true`. Falla en silencio durante meses. El arreglo son dos partes y la segunda casi nadie la pone: `service_role` tras verificar admin, **y aserción del número de filas afectadas** (0 filas = error, no éxito) — que además te cubre del `UPDATE` con el `WHERE` mal escrito, donde `service_role` no salva. Incluye grep para auditar un proyecto existente. |
| `seguridad-formularios` | **El formulario de tu cliente le está quemando el dominio.** Si el backend reenvía por correo, el spam que entra te tira la reputación y los leads de verdad acaban en spam. Cinco capas con código: honeypot (fuera de pantalla, **no** `display:none`, y al bot detectado se le manda a la página de gracias para que no aprenda), cronómetro anti-bot, sanitización en cliente **y** servidor, bloqueo de dominios desechables y límite por sesión — con el aviso honesto de qué protege de verdad cada una y qué es solo comodidad. |
