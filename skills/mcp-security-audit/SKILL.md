...
description: Auditoría de seguridad de servidores MCP — antes de instalar uno nuevo y antes de cada actualización. Triple defensa (pre-instalación / pre-actualización / vigilancia continua) para que solo entren al stack MCPs auditados y las actualizaciones no entren a ciegas. Activar en cualquier proyecto con MCPs configurados, especialmente al añadir uno nuevo o ejecutar npm update.
type: protocol-skill
---

# Skill: Auditoría de seguridad de MCPs

> Los MCPs son código de terceros que Claude ejecuta. Cada MCP es una superficie de ataque: prompt injection, ejecución arbitraria, secretos expuestos, supply chain attacks tipo "rug pull". Esta skill da un protocolo para que entren al stack solo MCPs auditados, y que las actualizaciones no entren a ciegas.
...
- Routine programada (mensual) de revisión de MCPs
- Tras supply-chain attack reportado en algún paquete del que dependan los MCPs

Si solo es seguridad de dependencias generales (npm/pip/composer) → usa una skill de auditoría de dependencias (p.ej. Snyk). Esta skill es **específica de MCPs**.

---

## Por qué esto importa (datos del sector, 2026)
...
- **Rug pull**: MCPs que mutan instrucciones DESPUÉS de instalar. Día 1 parece legítimo, una actualización silenciosa cambia el comportamiento (mismo patrón de ataques de supply-chain a paquetes npm populares).
...
| **L2 — Pre-actualización** | Antes de `npm update` o aceptar dependabot | Script wrapper con un scanner MCP |
| **L3 — Vigilancia continua** | Mensual o tras incidente sectorial | Routine programada que revisa updates pendientes |
...
> Por qué funciona mejor que un scanner automático: Claude entiende **intención**, no solo patrones. En auditorías con reglas YARA se detectaron 27 patrones sospechosos, **solo 6 eran riesgos reales** (los otros 21 falsos positivos). Claude distingue eso.
...
### Decisión de instalación — política recomendada
...
En lugar de `npm update`, lanza un wrapper que **bloquea el update si el scanner encuentra vulnerabilidades**. Ejemplo (bash):

```bash
#!/bin/bash
# safe-mcp-update.sh <ruta-proyecto-con-mcps>
cd "$1" || exit 1
npx @invariantlabs-ai/mcp-scan@latest . && npm update || echo "⛔ update CANCELADO: el scanner encontró riesgos"
```

El wrapper: (1) `cd` al proyecto, (2) corre el scanner, (3) si exit 0 (limpio) → `npm update`, (4) si exit != 0 → cancela y alerta.
...
### Routine programada (mensual)

Ejemplo de tarea mensual automatizada:

- Día 1 de cada mes, temprano
...
- Notificación (email / chat) con resumen y veredicto sugerido

### Tras incidente sectorial

Cuando aparezca en un feed de seguridad un caso de MCP comprometido:
...
## Ejemplo aplicado — auditoría inicial de un stack

| MCP | Comando | Origen | Veredicto |
|---|---|---|---|
| **Snyk** | `snyk mcp -t stdio` | Snyk Limited (oficial) | ✅ SEGURO (first-party empresa de seguridad) |
| **playwright** | `npx @playwright/mcp` | Microsoft (oficial) | ✅ SEGURO (first-party Microsoft) |
| **Google Drive** | (integración gestionada) | Anthropic (servicio gestionado) | ✅ SEGURO (no es MCP local, lo gestiona el proveedor) |

Patrón: los MCPs first-party de empresas reconocidas pasan rápido; los de terceros pasan por el Método 1A antes de entrar.
...
1. Invocar skill `mcp-security-audit`
...
Política: solo MCPs first-party de empresas reconocidas, o auditados
con el Método 1A de la skill antes de instalar.
```
...
- ❌ Dejar MCPs antiguos olvidados en configs legacy con credenciales que ya no rotas
